...
internal class P2PSession(
    ...
    fun subscribeToEvent(eventName: String) = engine.subscribeToP2PEvent(eventName)

    fun unsubscribeFromEvent(eventName: String) = engine.unsubscribeFromP2PEvent(eventName)

    fun applyDynamicConfig(config: DynamicCoreConfig) {
        engine.applyDynamicConfig(config)
    }
